...
import { Check, Sparkles, ArrowRight, Utensils, Coffee, Moon, Sun } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
                ...
                return (
                  ...
                  >
                    <div className="h-32 sm:h-40 relative overflow-hidden">
                      ...
                      {isActive && (
                        <span className="absolute top-3 right-3 bg-[#006B46] text-white font-title text-[8px] tracking-widest px-2.5 py-0.5 rounded-full uppercase font-bold border border-white/20">
                          ACTIVE
                        </span>
                      )}

                      <div className="absolute bottom-3 left-3 right-3 text-white space-y-0.5">
                        <h4 className="font-serif font-bold text-sm sm:text-base leading-snug">{cat.name}</h4>
                      </div>
                    </div>
                  </button>
                );
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-4 border-b border-[#006B46]/10">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-[#374151] font-light">
                {currentCategoryVisual.desc}
              </p>
            </div>

            {/* Diet Filter Switch */}
            <div className="flex items-center gap-1.5 bg-white p-1 rounded-full border border-[#006B46]/15 shadow-sm">
              <button
                onClick={() => setDietFilter('all')}
                className={`px-4 py-1.5 rounded-full font-title text-[9px] tracking-widest uppercase font-bold transition-all ${
                  dietFilter === 'all' 
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {filteredItems.map((dish) => (
                <div 
                  key={dish.id} 
                  className="editorial-card p-6 rounded-2xl bg-white flex flex-col justify-between space-y-4 group hover:border-[#006B46]/40"
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <h4 className="text-lg font-serif font-bold text-[#111827] group-hover:text-[#006B46] transition-colors leading-snug">
                        {dish.name}
                      </h4>
                      
                      <span className={`px-2.5 py-0.5 rounded-full text-[8.5px] font-title font-bold uppercase shrink-0 ${
                        dish.isVeg 
                  ...
                  <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                    <span className="text-[9px] font-title tracking-wider text-gray-400 uppercase font-semibold">
                    ...
                    <button
                      ...
                      className="font-title text-[9px] tracking-widest text-[#006B46] hover:underline uppercase font-bold flex items-center gap-1"
                    >
          ...
          <div className="mt-20 pt-16 border-t border-[#006B46]/15">
            <div className="text-center max-w-2xl mx-auto mb-12 space-y-2">
              <span className="font-title text-[9px] tracking-widest text-[#006B46] uppercase font-bold block">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {siteData.packages.map((pkg, idx) => (
                ...
                >
                  {pkg.popular && (
                    <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#006B46] text-white font-title text-[9px] tracking-widest uppercase font-bold px-4 py-1 rounded-full shadow-sm">
                      MOST POPULAR
                    </span>
                  )}
                  ...
                  <div className="space-y-4">
                    <div>
                      <h3 className="font-serif text-2xl font-bold text-[#111827]">{pkg.name}</h3>
                      <p className="text-xs text-[#374151] font-light mt-1">{pkg.subtitle}</p>
                    </div>

                    <div className="py-3 border-y border-gray-100 flex items-baseline gap-1">
                      <span className="text-3xl font-serif font-bold text-[#006B46]">{pkg.price}</span>
                      <span className="text-xs text-gray-400 font-sans">{pkg.priceUnit}</span>
                    </div>

                    <div className="space-y-2 pt-1">
                      {pkg.features.map((feat, fIdx) => (
                        <div key={fIdx} className="flex items-center gap-2 text-xs text-[#111827]">
                          <Check className="w-3.5 h-3.5 text-[#006B46] shrink-0" />
                          <span className="font-sans text-xs text-[#374151]">{feat}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="pt-8">
                    <button
                      ...
                      className="btn-editorial-outline py-3 rounded-full text-center block w-full text-[10px]"
                    >
          ...
          <div className="mt-16 text-center bg-white p-8 sm:p-10 rounded-2xl border border-[#006B46]/15 shadow-sm max-w-2xl mx-auto space-y-3">
            <h3 className="text-2xl font-serif font-bold text-[#111827]">Need a Custom Menu Combination?</h3>
            <p className="text-xs text-[#374151] font-light leading-relaxed">
            ...
            <div className="pt-2">
              <button
                ...
                className="btn-editorial-outline px-8 py-3.5 rounded-full text-[10px] inline-block font-bold"
              >
